...
// 🌟 샵 이름은 고정하되, 새로고침 시 무작위로 섞여서 5개만 노출될 전체 샵 목록
const allRegionalShops = [
...
export default function RegionalClientUI({ district, dongName }: RegionalClientUIProps) {
  const [displayShops, setDisplayShops] = useState<typeof allRegionalShops>([]);

  useEffect(() => {
    // 새로고침 시 샵을 랜덤으로 섞은 뒤 5개만 추출
    const shuffled = [...allRegionalShops].sort(() => Math.random() - 0.5);
