...
def generate_sitemap():
    ...
    # 4. 메인 페이지와 동일한 서울, 경기, 인천 지역 및 동 데이터 구조
    regionData = {
        "seoul": {
            "name": "서울특별시",
            "districts": {
    ...
    # 5. 메인 페이지 검색 로직과 동일하게 '시', '구', '군' 접미사 정제 후 URL 생성
    for region, reg_info in regionData.items():
        for district_key, dist_info in reg_info["districts"].items():
            raw_district_name = dist_info["name"]
            
            # 메인 페이지와 동일한 접미사 제거 로직 (예: '안산시 단원구' -> '안산시 단원')
            # 만약 '단원'처럼 완전히 떼고 싶으시다면 .replace('구', '').replace('시', '') 처리를 하시면 됩니다.
            import re
            clean_district_name = re.sub(r'(시|구|군)$', '', raw_district_name).strip()
            
            encoded_district = urllib.parse.quote(clean_district_name)
    ...
    print(f"🎉 메인 페이지 검색 정제 로직이 완벽 반영되어 총 {len(url_entries)}개의 URL이 public/sitemap.xml로 생성되었습니다!")
